  ...
  var Api = {
    ...
    getStart: function (options) {
      ...
      registrationPromise = postJson(GET_START_URL, {
        ...
        /* Falls back to the signed-in Odoo administrator's name/email
           (window.SkynetVideoSubtitleConfig.userName/userEmail, set by the
           dashboard template) before falling back to the bare domain —
           this is display/contact data only and has no effect on which
           domain gets registered. */
        name: opts.name || SKYNET_VS_CONFIG.userName || site,
        email: opts.email || SKYNET_VS_CONFIG.userEmail || "no-reply@" + site,
