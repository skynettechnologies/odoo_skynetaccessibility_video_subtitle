/* ==========================================================================
   api.js — live client for the AIOA staging video-subtitle API.

   Base: https://stagingada.skynettechnologies.us/api/website-video
...
(function (global) {
  ...
  /* ======================================================================
     THE ONE SWITCH — going live is a single edit.

     Delete the word "staging" from the host below:
         staging      https://stagingada.skynettechnologies.us
         production   https://ada.skynettechnologies.us

     Every endpoint URL is derived from it, so there is nothing else to
     change. Odoo also lets an administrator override this per-install from
     Settings > SkynetAccessibility Video Subtitle > Configuration (the
     `skynet_video_subtitle.site_origin` system parameter) — the dashboard
     template injects that value as window.SkynetVideoSubtitleConfig.siteOrigin
     before this file loads, and it wins over the hardcoded default below when
     present, so pointing an install at staging needs no code edit.
     ====================================================================== */
  var SKYNET_VS_CONFIG =
    (global.SkynetVideoSubtitleConfig && typeof global.SkynetVideoSubtitleConfig === "object")
      ? global.SkynetVideoSubtitleConfig
      : {};

  var SITE_ORIGIN = String(
    SKYNET_VS_CONFIG.siteOrigin || "https://ada.skynettechnologies.us"
  ).replace(/\/+$/, "");
  ...
  /* Sent as `platform` on registration, identifying where the install came
     from. Odoo's ir.config_parameter `skynet_video_subtitle.platform`
     (default "Odoo") is injected the same way as siteOrigin above. */
  var PLATFORM = String(SKYNET_VS_CONFIG.platform || "Odoo");
  ...
  /* ----------------------------------------------------------------------
     Live Odoo session lookup — ported from getOdooUserInfo() in the
     SkynetAccessibility Scanner plugin (skynet_scanner.js).

     dashboard_page.xml bakes a name/email into
     window.SkynetVideoSubtitleConfig at server-render time
     (request.env.user at the moment the page happened to render). That
     snapshot can go stale or wrong (e.g. it falls back to the seeded
     "Administrator" account whenever request.env.user has no email/login
     at render time — see the comments in dashboard_page.xml). This asks
     Odoo directly, client-side, right before get-start fires:

       1. POST /web/session/get_session_info — Odoo's own session endpoint,
          returns {uid, name, username} for whoever this browser is
          actually signed in as *right now*.
       2. POST /web/dataset/call_kw on res.users (search_read) with that
          uid — fills in the full name/login/email, same as the scanner
          plugin does.

     Both are standard Odoo JSON-RPC endpoints, so they run under this
     browser's current session cookie automatically — nothing to
     configure. If either step fails or there's no session, this
     resolves with whatever it already has and getStart() falls through
     to window.SkynetVideoSubtitleConfig, then the bare domain, exactly
     as before.
     ---------------------------------------------------------------------- */
  function fetchOdooUserInfo() {
    var name = "";
    var email = "";
    ...
    return fetch("/web/session/get_session_info", {
      ...
      .then(function (d) {
        var info = (d && d.result) || {};
        var uid = info.uid || null;
        name = info.name || "";
        email = info.username || "";

        if (!uid) return { name: name, email: email };
        ...
        return fetch("/web/dataset/call_kw", {
          ...
          .then(function (d2) {
            var u = ((d2 && d2.result) || [])[0] || {};
            if (u.name) name = u.name;
            if (u.email) email = u.email;
            else if (u.login && u.login.indexOf("@") !== -1) email = u.login;
            return { name: name, email: email };
          });
      })
      .catch(function () {
        /* No session / network error — let getStart() use its other
           fallbacks instead of failing registration outright. */
        return { name: name, email: email };
      });
  }
  ...
  var Api = {
    ...
    getStart: function (options) {
      ...
      /* SAFETY NOTE — read before calling this from anywhere new.
         ...
         registered site therefore never reaches that call site at all —
      ...
      registrationPromise = fetchOdooUserInfo().then(function (odooUser) {
        return postJson(GET_START_URL, {
          website_url: site,
          platform: opts.platform || PLATFORM,
          /* Live session lookup first (fetchOdooUserInfo — whoever is
             actually signed into this browser right now), then the
             dashboard template's server-rendered snapshot
             (window.SkynetVideoSubtitleConfig.userName/userEmail) as a
             secondary fallback, then the bare domain — this is
             display/contact data only and has no effect on which domain
             gets registered. */
          name: opts.name || odooUser.name || SKYNET_VS_CONFIG.userName || site,
          email: opts.email || odooUser.email || SKYNET_VS_CONFIG.userEmail || "no-reply@" + site,
